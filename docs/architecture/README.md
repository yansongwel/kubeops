...
KubeOps 是一个云原生、模块化单体架构的 Kubernetes 统一管理平台。系统架构遵循以下原则：

1. **模块化单体**：核心能力以内聚模块形式运行在同一进程
2. **API 优先**：所有功能通过良好定义的 API 访问
3. **可插拔组件**：日志、监控、AI 提供商可互换
4. **Kubernetes 原生**：为 K8s 构建，使用 K8s 原语
5. **可扩展性**：模块边界清晰，支持未来拆分与水平扩展
...
1. **接入层**：可选外部网关作为统一流量入口
2. **应用层**：前端 + 单体后端模块
3. **数据层**：K8s API、外部服务集成
...
```
用户 → 前端 → 网关模块 → Kube 管理模块 → Kubernetes API
  ←         ←          ←            ←              ←
```
...
- **方式**：进程内调用
- **边界**：通过接口与模块包边界保持隔离
- **外部通信**：对接 Jenkins/ArgoCD/AI Provider 等依旧使用其官方 API/SDK
